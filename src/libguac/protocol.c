...
int guac_protocol_get_image(guac_socket* socket, const char* type, uint32_t key, int x, int y, int width, int height) {
    int ret_val;

    guac_socket_instruction_begin(socket);
    ret_val =
           guac_socket_write_string(socket, "8.getimage,")
        || __guac_socket_write_length_string(socket, type)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, key)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, x)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, y)
...
int guac_protocol_put_image(guac_socket* socket, const char* type, uint32_t key, int x, int y, int dirty_x, int dirty_y, int dirty_width, int dirty_height) {
    int ret_val;

    guac_socket_instruction_begin(socket);
    ret_val =
           guac_socket_write_string(socket, "8.putimage,")
        || __guac_socket_write_length_string(socket, type)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, key)
        || guac_socket_write_string(socket, ",")
        ...
        || __guac_socket_write_length_int(socket, y)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, dirty_x)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, dirty_y)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, dirty_width)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, dirty_height)
        || guac_socket_write_string(socket, ";");
...
int guac_protocol_del_image(guac_socket* socket, const char* type, uint32_t key) {
    int ret_val;

    guac_socket_instruction_begin(socket);
    ret_val =
           guac_socket_write_string(socket, "8.delimage,")
        || __guac_socket_write_length_string(socket, type)
        || guac_socket_write_string(socket, ",")
        || __guac_socket_write_length_int(socket, key)
        || guac_socket_write_string(socket, ";");
